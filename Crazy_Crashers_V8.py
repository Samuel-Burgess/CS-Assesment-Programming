...
def load_high_score():
    """Load high score file if it exists"""
...
def save_high_score(score_input):
    """Save high score"""
...
def game_over_menu(score_input, high_score_input):
    """Show game over menu"""
    while True:
        SCREEN.fill(WHITE)
        SCREEN.blit(crash_art_stretched, (0, 0))
        draw_text(f"Game Over!", font_title,
                  WHITE, SCREEN, 40, 0)
        draw_text(f"Score: {score_input}", font_title,
                  WHITE, SCREEN, 60, 80)
        draw_text(f"High Score: {high_score_input}", font_subtitle,
                  WHITE, SCREEN, 45, 120)
        draw_text("Press R to restart", font_subtitle,
                  WHITE, SCREEN, 40, 350)
        draw_text("or Q to quit", font_subtitle,
                  WHITE, SCREEN, 70, 370)
        pygame.display.flip()
